...
Model.prototype.applyIndexes = function(callback){
  var indexLabel = this.label.split(":").slice(-1)[0];
  ...
  if(consLen || indLen){
    ...
    for(var j=0; j<indLen; j++){
      transStatement.push({statement: 'CREATE INDEX ON :' + indexLabel + '(' + self.schema._indexes[j] +')'});
    }
    ...
    self.db.beginTransaction({statements: transStatement}, {commit:true}, function(err){
      ...
      if(err){
        ...
        var filterIndexes = function(object){
          return object.statement !== 'CREATE INDEX ON :' + indexLabel + '(' + field +')';
        };
